...
function openModal(content) {
  ...
  document.body.style.filter = "blur(5px)"; // Apply blur to background
}
...
document.querySelectorAll('.level-link a').forEach(function(link) {
  link.addEventListener('click', function(event) {
    ...
    var messageHtml = levelItem.getAttribute('data-message');

    var content = `<h2>${levelName}</h2><p>ID: ${levelId}</p>${messageHtml}<p>More content here...</p>`;
    openModal(content);
  ...
  if (path) {
    document.querySelectorAll('.level-item').forEach(function(item) {
      if (item.getAttribute('data-name').toLowerCase() === path.toLowerCase()) {
        ...
        var messageHtml = item.getAttribute('data-message');
